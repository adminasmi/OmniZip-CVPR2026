...
def gen_coreml(space, attn_blk_cls, moa_branch='v', mode='cpu_only'):
    for s in space:
        size = s.pop('size')
        ...
        model = BaseRWKV_HIRA_MOA_MOE(args, attn_blk_cls=attn_blk_cls, use_mlp_moe=True)
        ...
        with torch.no_grad():
            ...
            save_dir = f'/home/zhaoy/OmniComp-MoA-MoE-cvpr2026/coreml/mlpackage/{mode}/rwkv7-hira-{moa_branch}moa-moe/rwkv7-hira-{moa_branch}moa-moe-{size}'
...
if __name__ == '__main__':
    ...
    # for batch_size in [1, 128]:
        ...
        if GEN_COLEML:
            # gen_coreml(space=space, attn_blk_cls=AttnBlk_VMoA, moa_branch='v', mode='all')
            # gen_coreml(space=space, attn_blk_cls=AttnBlk_RKVMoA, moa_branch='rkv', mode='all')
            gen_coreml(space=space, attn_blk_cls=AttnBlk_KVMoA, moa_branch='kv', mode='all')