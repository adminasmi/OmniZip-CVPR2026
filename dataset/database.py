...
class SQLDataset(TextBaseDataset):
    def __init__(
        self, args, word2id_dict, mode='train', logger=None
    ):
        super().__init__(args, word2id_dict, mode, logger)
        self.modality = 'database'
        # Override start_id with database modal token
        self.start_id = word2id_dict.get('<database>', 0)
...
if __name__ == '__main__':
    ...
    class ARGS:
        ...
        sql_file  : List[str] = field(default_factory=lambda: ['database/WikiSQL/train.sql', 'database/WikiSQL/test.sql'])
        sql_corpus: List[str] = field(default_factory=lambda: [f'spm_wikisql_train_bpe_{VOCAB_SIZE}_{COVERAGE}.txt', f'spm_wikisql_test_bpe_{VOCAB_SIZE}_{COVERAGE}.txt'])
        sql_unk  : List[str] = field(default_factory=lambda: [f'unk_wikisql_train_bpe_{VOCAB_SIZE}_{COVERAGE}.txt', f'unk_wikisql_test_bpe_{VOCAB_SIZE}_{COVERAGE}.txt'])
    args = ARGS()
