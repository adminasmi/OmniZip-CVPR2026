...
// Set RWKV_DEBUG_PRINT to 1 to enable debugging prints, or 0 to disable.
// Example:
//   nvcc -DRWKV_DEBUG_PRINT=1 ...
// -----------------------------------------------------------------------------
...
__global__ void kernel_forward(
        ...
        float *__restrict__ const _saa, 
        float *__restrict__ const _sss
    )
{
...
#if RWKV_DEBUG_PRINT
    if (h * _N_ >= C) {
        printf("[ERROR] h*N >= C: h=%d, _N_=%d, C=%d\n", h, _N_, C);
        return;
    }
    if (e >= B) {
        printf("[ERROR] e >= B: e=%d, B=%d\n", e, B);
        return;
    }
#endif
...
#if RWKV_DEBUG_PRINT
    RWKV_CHECK_BOUNDS(t, B * T * C, "_v read");
#endif
    ...
    for (int _t = 0; _t < T; _t++)
    {
        ...
        // __syncthreads();
#if RWKV_DEBUG_PRINT
    RWKV_CHECK_BOUNDS(t, B * T * C, "r,w,k,a,b");
#endif
        r[i] = float(_r[t]);
        float tmp = fminf(fmaxf(float(_w[t]), -60.0f), 60.0f);
        w[i] = __expf(-__expf(tmp));
        k[i] = float(_k[t]);
        a[i] = float(_a[t]);
        b[i] = float(_b[t]);
        check_float_value(r[i], "_r[t]", t);
        check_float_value(w[i], "_w[t]", t);
        check_float_value(k[i], "_k[t]", t);
        check_float_value(a[i], "_a[t]", t);
        check_float_value(b[i], "_b[t]", t);
...
#if RWKV_DEBUG_PRINT
        RWKV_CHECK_BOUNDS(t, B * T * C, "_saa write");
        check_float_value(sa, "sa (sum)", t);
#endif

        _saa[t] = float(sa);

        float vv = v[_t];
        check_float_value(vv, "v[_t]", _t);
        ...
        for (int j = 0; j < _N_; j++)
        {
            ...
            check_float_value(s, "state[j]", j);
            y += s * r[j];
        }
#if RWKV_DEBUG_PRINT
        RWKV_CHECK_BOUNDS(t, B * T * C, "_y write");
        check_float_value(y, "y (output)", t);
#endif
        _y[t] = F(y);
        ...
        if ((_t+1) % _CHUNK_LEN_ == 0)
        {
            ...
            {
#if RWKV_DEBUG_PRINT
    RWKV_CHECK_BOUNDS((p + j * _N_), B * C * _N_ * c, "_sss write");
    check_float_value(state[j], "state->sss", (p + j * _N_));
#endif
...
template <typename F>
__global__ void kernel_backward_zzz(
        const int B, 
        const int T, 
        const int C, 
        const int H,
    ...
    )
{
    ...
    for (int _t = T-2; _t > _CHUNK_LEN_-1; _t--)
    {
        ...
        float tmp = fminf(fmaxf(float(_w[t+C]), -60.0f), 60.0f);
        w[i] = __expf(-__expf(tmp));
    ...
    const F *__restrict__ const _gy, F *__restrict__ const _gr, F *__restrict__ const _gw, F *__restrict__ const _gk, F *__restrict__ const _gv, F *__restrict__ const _ga, F *__restrict__ const _gb)
{
    ...
    const int n_chunk = T / _CHUNK_LEN_;
    ...
    if (chunk != n_chunk - 1)
    {
        ...
        for (int i = 0; i < _N_; i++) 
        {
            ...
            a[i] = float(_a[p+i]);
            b[i] = float(_b[p+i]);
            float tmp = fminf(fmaxf(float(_w[p+i]), -60.0f), 60.0f);
            w[i] = __expf(-__expf(tmp));
    ...
    for (int _t = _CHUNK_LEN_-1; _t > -1; _t--)
    {
        ...
        for (int n = 0; n < _N_; n++)
        {
            ...
            // float tmp = fminf(fmaxf(float(_w[b_t_h+n]), -60.0f), 60.0f);
            // ww[n] = -__expf(tmp);
            // w[n]  = __expf(ww[n]);
